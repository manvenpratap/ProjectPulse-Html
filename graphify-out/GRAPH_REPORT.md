...
2. `build_umi()` - 19 edges
3. `Active Dropdown Options` - 19 edges
...
Cohesion: 0.18
Nodes (20): add_callout(), add_data_table(), add_document_control(), add_footer(), add_horizontal_rule(), add_page_break(), add_schema_table(), add_screenshot() (+12 more)
...
Cohesion: 0.21
Nodes (17): build_umi(), ch_audit(), ch_config(), ch_defects(), ch_delivery(), ch_gantt(), ch_getting_started(), ch_insights() (+9 more)
...
Cohesion: 0.13
Nodes (14): 1. Metric Caching (`buildDashCache`), 2. Earned Value Management (EVM), 3. Burn-Down & Burn-Up Projection Chart, 3. Burn-up & Burn-down Projections, 4. Weekly Velocity, 5. Cumulative Flow Diagram (CFD), Hover Tooltip, Interactive Module Filtering (+6 more)
...
## Knowledge Gaps
- **157 isolated node(s):** `capture_widgets.py ================== Captures element-specific cropped screensh`, `build_pcd.py ============ Builds the ProjectPulse Product Capabilities Document`, `build_fsd.py ============ Builds the ProjectPulse Functional Specification Docum`, `fs`, `path` (+152 more)
