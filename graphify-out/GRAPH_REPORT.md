...
## Corpus Check
- 20 files · ~317,699 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 225 nodes · 320 edges · 21 communities (20 shown, 1 thin omitted)
- Extraction: 100% EXTRACTED · 0% INFERRED · 0% AMBIGUOUS
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `687eb4ef`
...
2. `make_heading()` - 16 edges
3. `add_horizontal_rule()` - 15 edges
4. `add_data_table()` - 15 edges
5. `add_page_break()` - 15 edges
6. `12 Change Log Baseline` - 15 edges
7. `make_body()` - 14 edges
8. `add_screenshot()` - 14 edges
9. `add_callout()` - 14 edges
10. `make_bullet()` - 13 edges
...
Cohesion: 0.1
Nodes (19): 1. Codebase Architecture & File Map, 2. Global State Schema (`P` Object), 4. Core Function Registry, 5. Storage & Persistence Tiers, 6. Authoritative Business Rules, 7. Change Log (Milestones), A. Core Data Arrays, A. Task Status Transitions (+11 more)
...
## Knowledge Gaps
- **132 isolated node(s):** `fs`, `path`, `PROJECT_ROOT`, `HTML_FILE`, `STATUS_MD_FILE` (+127 more)
...
- **Why does `ProjectPulse — High-Density AI Context Pack` connect `Architecture & Audit & State` to `Scheduler & The & Heuristic`?**
  _High betweenness centrality (0.014) - this node is a cross-community bridge._
- **Why does `3. Entity Schemas` connect `Scheduler & The & Heuristic` to `Architecture & Audit & State`?**
...
- **Should `Changelog & 2026 & 05` be split into smaller, more focused modules?**
