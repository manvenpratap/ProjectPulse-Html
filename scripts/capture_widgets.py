...
capture_widgets.py
==================
Captures element-specific cropped screenshots of all 22 dashboard widgets.
Saves them into docs/screenshots/widgets/.
"""
...
OUT_DIR = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "..", "docs", "screenshots", "widgets")
)
...
WIDGET_SELECTORS = {
    "summary":               ".dash-widget-wrapper[data-id=\"summary\"]",
    "health":                ".dash-widget-wrapper[data-id=\"health\"]",
    "mgmt_insights":         ".dash-widget-wrapper[data-id=\"mgmt_insights\"]",
    "exec_reporting":        ".dash-widget-wrapper[data-id=\"exec_reporting\"]",
    "exec_capacity":         ".dash-widget-wrapper[data-id=\"exec_capacity\"]",
    "module_matrix":         ".dash-widget-wrapper[data-id=\"module_status_matrix\"]",
    "module_health":         ".dash-widget-wrapper[data-id=\"module_health\"]",
    "feature_insights":      ".dash-widget-wrapper[data-id=\"feature_insights\"]",
    "screen_status":         ".dash-widget-wrapper[data-id=\"screen_status\"]",
    "release_timeline":      ".dash-widget-wrapper[data-id=\"release_timeline\"]",
    "milestone_timeline":    ".dash-widget-wrapper[data-id=\"milestone-timeline\"]",
    "burndown":              ".dash-widget-wrapper[data-id=\"burndown\"]",
    "velocity":              ".dash-widget-wrapper[data-id=\"velocity\"]",
    "cfd":                   ".dash-widget-wrapper[data-id=\"cfd\"]",
    "overdue":               ".dash-widget-wrapper[data-id=\"overdue\"]",
    "high_impact_entities":  ".dash-widget-wrapper[data-id=\"high_impact_entities\"]",
    "workload":              ".dash-widget-wrapper[data-id=\"workload\"]",
    "cat_hours":             ".dash-widget-wrapper[data-id=\"cat_hours\"]",
    "defect_intel":          ".dash-widget-wrapper[data-id=\"defect_intel\"]",
    "intelligence":          ".dash-widget-wrapper[data-id=\"intelligence\"]",
    "analytics":             ".dash-widget-wrapper[data-id=\"analytics\"]",
    "activity":              ".dash-widget-wrapper[data-id=\"activity\"]",
}

def main():
    print(f"Capturing individual widget cropped screenshots...")
    ...
    with sync_playwright() as p:
        ...
        page.evaluate("() => { try { setView('dash'); } catch(e) { P.view='dash'; render(); } }")
        page.wait_for_timeout(3000)

        # Iterate over widgets and capture
        for wid, sel in WIDGET_SELECTORS.items():
            try:
                ...
                if elem.is_visible():
                    ...
                    print(f"  ✗  {wid} — element not visible")
            except Exception as ex:
                print(f"  ✗  {wid} — {ex}")

        # Now handle report-specific ones by switching the dashboard layout to render them
        print("\nSwitching dashboard layout to render report widgets...")
        page.evaluate("""() => {
            P.dashLayout = [
                { id: 'kpi_summary', on: true },
                { id: 'activities', on: true },
                { id: 'achievements', on: true },
                { id: 'risks', on: true },
                { id: 'plans', on: true },
                { id: 'team', on: true },
                { id: 'util', on: true },
                { id: 'overdue_tbl', on: true }
            ];
            setView('dash');
            render();
        }""")
        ...
        report_widgets = ["kpi_summary", "activities", "achievements", "risks", "plans", "team", "util", "overdue_tbl"]
        for wid in report_widgets:
            try:
                sel = f'.dash-widget-wrapper[data-id="{wid}"]'
                elem = page.locator(sel).first
                if elem.is_visible():
                    elem.scroll_into_view_if_needed()
                    page.wait_for_timeout(500)
